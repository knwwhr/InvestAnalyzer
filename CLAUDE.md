...
│                                                              │
│ 등급 산정 (100점 만점, 백테스트 기반 재정의) ⭐ v3.7 NEW    │
...
추천 등급 (v3.7 백테스트 기반 재정의):
S: 25-41 (최우선 매수 - 선행 신호, 승률 89.3%)
A: 42-57 (적극 매수 - 진입 적기, 승률 77.8%, 평균 +27.5%)
B: 58-88 (매수 고려 - 추세 진행, 승률 86.7%)
C: 89+ (과열 경고 - 단기 차익)
D: <25 (관망 - 신호 부족)
```
...
#### 추천 등급 (85점 만점, 백테스트 기반 재정의) ⭐ v3.7 NEW

**백테스트 검증 결과를 반영한 새로운 등급 체계:**
...
- **C등급** (89점 이상): ⚠️ 과열 경고 (단기 차익)
  - 모든 지표 점등, 단기 차익 또는 조정 대기
  - **백테스트**: 승률 100%, 평균 +8.1% (샘플 부족, 신뢰도 낮음)

- **D등급** (25점 미만): ⚫ 관망 (신호 부족)
  - 선행 지표 미감지, 관망 권장

**핵심 인사이트**: "점수가 낮을수록 진짜 선행 신호!"
...
- 🔥 **등급 체계 재정의** (백테스트 결과 기반) ⭐ 핵심 개선!
  - **C등급 역설 해결**: C등급(25-41점)이 실제로는 최고 승률(89.3%)
  - **등급 의미 역전**:
    - S등급 (25-41점): 선행 신호, 최우선 매수 (승률 89.3%, 평균 +24.9%)
    - A등급 (42-57점): 진입 적기 (승률 77.8%, 평균 +27.5% 최고!)
    - B등급 (58-88점): 추세 진행 (승률 86.7%, 평균 +24.9%)
    - C등급 (89+점): 과열 경고, 단기 차익 (샘플 부족)
  - **핵심 인사이트**: "점수가 낮을수록 진짜 선행 신호" (거래량/기관 진입 전)
...
**Version**: 3.7 (백테스트 검증 & 등급 체계 재정의)
**Author**: Claude Code with @knwwhr

**✨ "점수가 낮을수록 진짜 선행 신호!" - 백테스트로 검증된 새로운 등급 체계**
